...
dataset_dir_2 = 'training_set_codalab/train set 2'
train_set = os.path.join(dataset_dir_2, 'train.npy')
test_set = os.path.join(dataset_dir, 'test.npy')


def evaluation_score(y_pred, y_test):
    y_pred = np.squeeze(y_pred)
    assert y_pred.shape == y_test.shape, "Error: the shape of your prediction doesn't match the shape of ground truth " \
                                         "label. "
...
print("Accuracy: {}, F-measure: {}, Precision: {}, Recall: {}, Final_Score: {}".format(acc, f1, precision, recall,
                                                                                       final_score))
